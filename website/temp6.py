...
# Constants
MAX_POINTS = 20  # Maximum number of points to display on the graph
...
data_connections = deque(maxlen=100)
data_lock = Lock()
...
def collect_data():
    while True:
        ...
        with data_lock:
            data_connections.append({'timestamp': timestamp, 'connection_count': connection_count})
        time.sleep(2)
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.H2("Network Connections Over Time"),
            dcc.Graph(id='connections-graph'),
            dcc.Interval(
                id='connections-interval-component',
                interval=2*1000,
                n_intervals=0
            ),
        ], style={'width': '48%', 'display': 'inline-block'}),
...
def update_idle_graph(n):
    ...
    # Limit the number of points displayed on the graph
    times = times[-MAX_POINTS:]
    activity = activity[-MAX_POINTS:]
...
# Define the callback to update the network connections graph
@app.callback(
    Output('connections-graph', 'figure'),
    [Input('connections-interval-component', 'n_intervals')]
)
def update_connections_graph(n):
    with data_lock:
        timestamps = [item['timestamp'] for item in data_connections]
        connection_counts = [item['connection_count'] for item in data_connections]

    # Limit the number of points displayed on the graph
    timestamps = timestamps[-MAX_POINTS:]
    connection_counts = connection_counts[-MAX_POINTS:]
